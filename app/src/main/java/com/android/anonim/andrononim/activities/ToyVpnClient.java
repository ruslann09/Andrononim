package com.android.anonim.andrononim.activities;
...
import com.android.anonim.andrononim.R;
...
public class ToyVpnClient extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        findViewById(R.id.disconnect).setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                ToyVpnClient.this.startService(ToyVpnClient.this.getServiceIntent().setAction(ToyVpnService.ACTION_DISCONNECT));
            }
        });
    }
    ...
    private boolean checkPackages(Set<String> packageNames) {
        final boolean hasCorrectPackageNames = packageNames.isEmpty() ||
                getPackageManager().getInstalledPackages(0).stream()
                        .map(new Function<PackageInfo, Object>() {
                            @Override
                            public Object apply(PackageInfo pi) {
